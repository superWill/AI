...
OLD_DEVICE = "stm328s003_mcu_8pin"
OLD_SYMBOL = "stm328s003_sym_8pin"
OLD_SYMBOL_PATH = "SYMBOL/stm328s003_sym_8pin.esym"

NEW_DEVICE = "3280030000000001"
NEW_SYMBOL = "3280030000000002"
NEW_SYMBOL_PATH = f"SYMBOL/{NEW_SYMBOL}.esym"
...
def normalize_project(project):
    devices = project.setdefault("devices", {})
    old_device = devices.pop(OLD_DEVICE, {})
    devices[NEW_DEVICE] = {
        **old_device,
        "uuid": NEW_DEVICE,
        "title": "STM328S003",
        ...
        "attributes": {
            **(old_device.get("attributes") or {}),
            ...
            "Number of Pins": "8",
            "Name": "STM328S003",
            "Description": "8-pin MCU placeholder; footprint TBD",
        },
        "source": "codex-added",
        "version": "20260525",
        "symbol_type": 2,
    ...
    symbols = project.setdefault("symbols", {})
    symbols.pop(OLD_SYMBOL, None)
    symbols[NEW_SYMBOL] = {
        "uuid": NEW_SYMBOL,
        "title": "STM328S003",
...
def normalize_sheet(records, x, y):
    found = False
    for record in records:
        if record and record[0] == "COMPONENT" and len(record) > 4:
            attrs = [
                r
                for r in records
                if r and r[0] == "ATTR" and len(r) > 4 and r[2] == record[1]
            ]
            if any(r[3] == "Device" and r[4] in {OLD_DEVICE, NEW_DEVICE} for r in attrs):
                record[2] = "STM328S003.1"
                record[3] = x
                record[4] = y
                found = True
                ...
                break
    if not found:
        raise ValueError("STM328S003 component placement was not found")

    for record in records:
        if record and record[0] == "ATTR" and len(record) > 4 and record[2] == component_id:
            if record[3] == "Device":
                record[4] = NEW_DEVICE
            elif record[3] == "Symbol":
                record[4] = NEW_SYMBOL
            elif record[3] == "Convert to PCB":
                record[4] = "no"
            elif record[3] == "Designator":
                record[4] = "U99"
                record[6] = 1
                record[7] = x - 95
                record[8] = y + 62
...
def convert(input_path: Path, output_path: Path, x: int, y: int):
    with zipfile.ZipFile(input_path) as source:
        ...
        with zipfile.ZipFile(output_path, "w", compression=zipfile.ZIP_DEFLATED) as target:
            ...
            for info in source.infolist():
                ...
                elif info.filename == NEW_SYMBOL_PATH:
            ...
            if NEW_SYMBOL_PATH not in existing_names:
                target.writestr(NEW_SYMBOL_PATH, dump_records(symbol_records()))
...
def main():
    ...
    component_id = convert(args.input, args.output, args.x, args.y)
    print(json.dumps({"component_id": component_id, "device": NEW_DEVICE, "symbol": NEW_SYMBOL}, indent=2))
