# baby-sense-research — 宝宝哭闹排查助手

定位（PRD v0.5）：宝宝哭了 → 录 5–15s + 填上下文 → 给排查建议 → 家长**选一个动作去做** → **过几分钟追问是否缓解** → 记住"动作→结果" → 下次按这个宝宝的历史重排建议。

> 🎯 核心：闭环不是"判断宝宝为什么哭"，而是"**帮家长排查，并记住什么办法对这个宝宝有效**"。哭声分析只是输入之一。**v1 不下任何 ML 判断**（不报原因、不报要不要紧）。
...
产品决策见 [`婴儿哭声感知-方案与PRD.md`](婴儿哭声感知-方案与PRD.md)（PRD v0.5）；工程见 [`技术方案.md`](技术方案.md)（v0.3）；**使用闭环**见 [`闭环与建议引擎设计.md`](闭环与建议引擎设计.md)；架构监督见 [`架构监督看板.md`](架构监督看板.md)；UX：[`v1-结果页文案与交互.md`](v1-结果页文案与交互.md) · [`v1-页面流原型.md`](v1-页面流原型.md) · [`v2-档案页设计.md`](v2-档案页设计.md)；数据见 [`数据清单.md`](数据清单.md)。
